...
public class WindowsKeyHook : Singleton<WindowsKeyHook>
{
    ...
    private struct KeyboardInfo
    {
        ...
        public KeyboardInfo(VirtualKeys key)
        {
            this.Key = key;
        }
    }
    ...
    private static extern IntPtr SetWindowsHookEx(HookType hookType, WindowsHookDelegate lpfn, IntPtr hMod, uint dwThreadId);
    ...
    public KeyboardEvent OnKey = new KeyboardEvent();
    ...
    private void InstallLowLevelKeyboardHook()
    {
        ...
        KEYBOARD_HANDLE = SetWindowsHookEx(HookType.WH_KEYBOARD_LL, RecieveKeyEvent, hModule, 0);
    ...
    private static IntPtr RecieveKeyEvent(int code, IntPtr wParam, IntPtr lParam)
    {
        if (code >= 0)
        {
            ...
            if (typeCode == 256)
            {
                KEYBOARD_QUEUE.Enqueue(new KeyboardInfo((VirtualKeys)(vkCode)));
            }
        }
