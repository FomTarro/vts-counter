...
namespace VTS.Core
{
	...
	public class VTSWebSocket : IVTSWebSocket
	{
		...
		private Action<int> _onPortDiscovered;
		public Action<int> OnPortDiscovered
		{
			get { return this._onPortDiscovered; }
			set { this._onPortDiscovered = value; }
		}
		...
		public void Initialize(IWebSocket webSocket, IJsonUtility jsonUtility, IVTSLogger logger)
		{
			if (this._ws == null)
			{
				...
				GLOBAL_PORT_DISCOVERY_EVENT += OnPortDiscoveredHandler;
			}
		...
		private void OnPortDiscoveredHandler(IPAddress address, int port)
		{
			if (MapAddress(address).Equals(MapAddress(this._ip)))
			{
				...
				if (this.OnPortDiscovered != null)
				{
					this.OnPortDiscovered.Invoke(port);
				}
			}
		}
